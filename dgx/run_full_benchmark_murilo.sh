...
#SBATCH --output=/raid/murilo/projects/info-gainme/logs/%x-%j.log

# Versão do run_full_benchmark.sh adaptada para o usuário `murilo`.
# Diferenças em relação ao original:
#   - PROJECT_DIR / SINGULARITY_IMAGE / HF cache apontam para /raid/murilo
#   - SHARED_GROUP é overridável (default: grupo primário do usuário)
#   - Todos os bind mounts do singularity refletem /raid/murilo → /workspace
#
# Override comum via --export=ALL,KEY=VAL,...
#   PROJECT_DIR        (default /raid/murilo/projects/info-gainme)
#   SINGULARITY_IMAGE  (default /raid/murilo/images/vllm_openai_latest.sif)
#   SHARED_GROUP       (default $(id -gn) — grupo primário do usuário que submeteu)
#   RAID_ROOT          (default /raid/murilo) — usado para os binds

umask 002

# ============================================
# User-specific paths (override via --export se necessário)
# ============================================
RAID_ROOT="${RAID_ROOT:-/raid/murilo}"
PROJECT_DIR="${PROJECT_DIR:-${RAID_ROOT}/projects/info-gainme}"
SINGULARITY_IMAGE="${SINGULARITY_IMAGE:-${RAID_ROOT}/images/vllm_openai_latest.sif}"
SHARED_GROUP="${SHARED_GROUP:-$(id -gn)}"
...
export LOGS_DIR="/workspace/projects/info-gainme/logs"
...
echo "=========================================="
echo "Info Gainme Full Benchmark (murilo) - $(date)"
echo "RAID_ROOT:        ${RAID_ROOT}"
echo "PROJECT_DIR:      ${PROJECT_DIR}"
echo "SINGULARITY:      ${SINGULARITY_IMAGE}"
echo "SHARED_GROUP:     ${SHARED_GROUP}"
...
for CONFIG in "${CONFIGS[@]}"; do
    ...
    sg "${SHARED_GROUP}" -c "singularity exec --bind ${RAID_ROOT}:/workspace --pwd /workspace/projects/info-gainme '${SINGULARITY_IMAGE}' bash -c \"pip install --user -r requirements.txt 2>/dev/null; python3 benchmark_runner.py --config '${REL}' --servers-override '${SERVERS_OVERRIDE}' ${RUNS_ARG}\"" && echo "  ✓" || echo "  ✗"
    echo ""
done
